...
async function init() {
    ...
    responses.forEach(response => {
        response.addEventListener('click', () => { testResponse(response.src); });
    });
...
function generateQuestion() {
    ...
    if (questionNumber.textContent != totalQuestions.textContent) {
        ...
        responses[randomPosRes].src = response;
        responsesNotFilled.forEach(response => {
            response.src = generateCountry().flag;
        });
    } else endGame();
}
...
function endGame() {
    console.log('End of the game\n' +
        ...
        responses.forEach(response => {
            response.removeEventListener('click', () => {testResponse(response.src)})
        });
}
